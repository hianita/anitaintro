...
def  signin ():
    ...
    if result[1]==request.values["username"] and result[2]==request.values["password"]:
...
@app.route("/api/user")
def lookup():
    lookupname = request.args.get("username", None)
    mycursor.execute(f"SELECT * FROM user where username='{lookupname}'")
    ...
    if myresult is None:
        ...
        username = myresult[2]
        pkg = {}
        content = {'id':id,'name':name,'username':username}
        pkg['data'] = content
        return json.dumps(pkg, ensure_ascii=False)
